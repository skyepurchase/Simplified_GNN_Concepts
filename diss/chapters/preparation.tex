...
To motivate the idea of the SGC consider applying a parameterised convolutional filter to the graph data rather than a parameterised function. A convolution is a multiplication, a signal, represented by the node representation\footnote{The signal is a function $\vh: \sV \to \R^n$ but, given nodes are fixed, define $\vh_i \equiv \vh(v_i)$.}
