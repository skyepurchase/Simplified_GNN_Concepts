...
Additionally, local \texttt{Tensorboard}\note{citation needed?} instances help visualise and analyse progress of experiments in realtime, \texttt{Git} and \texttt{Github} implement version control with \texttt{scp} and the \texttt{Student Run Computing Facility} as a third backup for experiment results.
...
\texttt{PyTorch} was chosen over \texttt{Tensorflow}\note{citation needed} as it is more ``pythonic'' and \texttt{PyTorch Geometric} is a simpler framework to use.
...
\texttt{Hyperopt}\note{citation needed} is also used as \textit{Wu et al.}\cite{wu2019simplifying} state this method was used to find the weight decay hyperparameter.

For scientific computing when analysing concepts I use \texttt{Numpy} and \texttt{scikit-learn}\note{citation neede}. For the visualisation of graphs I use \texttt{NetworkX}\note{citation needed} to draw the graphs and \texttt{Matplotlib}\note{citation needed} to visualise and store the result, I also use \texttt{Matplotlib} to visualise hyperparameter surfaces. Note that \texttt{NetworkX} is included in the \texttt{PyTorch Geometric} library.

For unit tests I use Python's own \texttt{unittest} library and to help with software development I use Python's \texttt{typing} library alongside the built in typing functionality of the other libraries. Additionally I use \texttt{tqdm}\note{citation needed?} to visualise progress during concept analysis.
