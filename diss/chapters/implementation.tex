...
\texttt{PyTorch}\note{citation needed} provides this functionality through \emph{hooks} which are functions that can be attached to modules within a model.
...
Rather than implement my own k-means algorithm I utilise the algorithm already present within \texttt{scikit learn}\note{citation needed}.
...
This is achieved by fitting a decision tree\note{citation needed} as proposed by\note{citation needed} to the concept data and labels.
...
This process is also used for REDDIT-BINARY\note{citation needed}.

In the case of the Mutagenicity\note{citation needed} dataset there already exists a standard chemical colour scheme for atoms\note{citation needed}.
...
Instead, I define pre-determined values for each atom based on the chemical colour scheme\note{citation needed} giving a better comparison across concepts.
...
\begin{itemize}
    ...
    \item 
        \textbf{Concept evaluation}
        Though parts of this implementation use algorithms provided by \texttt{scikit learn}\note{citation needed} the results of these functions needs to be verified in the new context.
