...
This can be mitigated by calculating an upperbound on the GED in $\mathcal{O}(n^3)$ however this would negatively impact the purity scores of Barabasi-Albert concepts.
...
\begin{itemize}
    ...
    \item[] 
        ...
        However, to verify that these are correctly implemented small test graphs are used.
    \item[] 
...
The new SGC mdoel has an additional pooling layer to create a single representation for each graph.
...
\label{sec:SGCN}
\paragraph{Motivation}
The derivation in \Sref{sec:SGC} shows that SGC is directly derived from GCN using the same graph filter and so the two models should be interchangeable.
Futhermore, the benefit of SGC is to reduce the complexity of GCN and pre-compute the graph operation.
Though very small datasets have little improvement on cost or reduction in parameters this is not the case for larger datasets.
...
A linear layer is then added to the standard SGC to reduce this concatenated dimension, during this JSGC is able to infer more complex graph structure than SGC.
