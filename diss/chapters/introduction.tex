...
\error{Include a figure comparing linear to non-linear GNNs.}
...
\fig{concept}{An example graph concept from GCN demonstrating that when classifying the top node the model identifies the house structure and attaching arm. The consistency of the structure suggests that this is important to classifying the top node.}
...
\error{Properly describe each paper's aims/results/contributions. Include all the major work that has led up to this point and any similar explorations.}
