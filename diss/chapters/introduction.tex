...
This is becauase, unlike standard NNs, the power of GNNs is hypothesised to arise from utilising the graph topology rather than approximating arbitrary functions on feature data.
Thus the ability for deep NNs to approximate functions through non-linearity may not be required.
Instead \emph{simplified GNNs} have been proposed~\cite{chanpuriya2022simplified,chien2020adaptive,wu2019simplifying} which are either fully linear or quasi-linear, using one or two non-linear layers.
...
This demonstrates that the neighbouring nodes and attaching arm are influence to classification of the highlighted green node the most.
...
SGC is chosen as the linear model to compare to the non-linear GCN, furthermore the proposed SGCN (\Sref{sec:SGCN-imp}) extends SGC by reintroducing some non-linearity from GCN.
...
\begin{enumerate}
    ...
    \item
        ...
        Furthermore the concept importance matches or surpasses the scores achieved by GCN.
\end{enumerate}
