...
To speed up the hyperparameter sweep I utilised a Google Colab Pro account with 1 hyperthreaded Intel Xeon Processor @ 2.3GHz with 1 core and 12 Gigabytes of Ram.
...
Mutagenicity is an outlier, achieving 0.626 compared to 0.967, as extracting concepts requires a single forest of graphs be used instead of individual graphs.
The resulting accuracy drops to 67.1\% and is likely the cause for the low accuray.
...
In contrast to completeness, average purity has little correlation between the two results this is due to the 13-node cut-off and non-deterministic nature of $k$-Means clustering.
%rather than an incorrect implementation.


\begin{figure}
    \centering
    \includegraphics[width=0.49\textheight]{figures/GCN-BA-Shapes}
    \includegraphics[width=0.49\textheight]{figures/Magister-BA-Shapes}
...
SGC does not perform significantly better than random guesses except in the cases of BA Shapes, 61.4\% compared to 25\%, and BA Grid, 72.4\% compared to 50\%.
...
Concept 5 demonstrates a shortcoming of calculating purity: this is considered a ``pure'' concept, the Barabasi-Albert graphs are not considered in calculation, but is clearly impure as the house structure and BA structure are not isomporphic. 

Concepts 2, 3 and 4 demonstrate that SGC can identify pure concepts with important graph structure components clearly demonstrating the identification of the house structure and attaching arm.
...
In comparison, GCN can distinguish between members in the same community demonstrated by Concepts B and C.
...
GCN always treats the two communities differently regardless of structural similarity which is clear when comparing Concepts D and E to Concept 3.
...
A smaller subset of the datasets are considered looking at BA Shapes, as the best-performing dataset, and BA Community, as the worst.
...
GCN exceeds SGC in accuracy, achieving 93.6\% in comparison to 61.6\%, and completeness, achieving 0.967 compared to 0.537.
...
Concepts 1 and 2 suggest the identification structure though consistently focus on single atoms, sulphur and nitrogen, whereas Concepts A and B show consistent multi-atom structures.
Concept 3 highlights the focus on grouping by atom as chlorine is the consistent feature across subgraphs.
...
SGCN achieves a higher completeness score across all layers and highlights a major benefit of the model in maintaining beneficial properties.

An analysis of the concepts extracted for SGCN is presented in \Aref{app:concepts}.
...
Most importantly all concepts, except for the single error in Concept 1, show consistent labelling throughout, particularly in Concept 3 where all nodes are ``outside'' nodes.
