...
Drawback \ref{nb:accuracy} motivates the additional evaluation metric of accuracy as \Sref{sec:comparison} demonstrates that SGC does not meet the desired accuracy.
...
This fulfills the $2^{nd}$ success criterion.
...
However, as discussed in \Sref{sec:evaluation}, the calculation of purity scores and quantitative analysis are not sufficient when comparing purity scores.
Due to the the 13 node cut-off and non-deterministic nature of k-Means\note{citation needed} clustering (with no average values provided by \textit{Magister et al.}) the exact concept purity scores are likely to behave erratically as demonstrated.
...
%    \caption{A side-by-side comparison of BA Shapes concepts discovered. The left figure demonstrates the concepts from figures 2, 3 and 5 from \textit{Magister et al.}\cite{magister2021gcexplainer} and the right figure demonstrates the reproductions. Each row represents an individual concept and the concepts are labelled}
%    \label{fig:test}
%\end{figure}
