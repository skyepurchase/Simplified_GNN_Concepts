...
\error{Include and analysis of Mutagenicity}
...
As discussed in \Sref{sec:evaluation} the calculation of purity scores and quantitative analysis are not sufficient when comparing purity scores.
Due to the the 13 node cut-off and non-deterministic nature of clusters (with no average values provided by \textit{Magister et al.}) the exact concept purity scores are likely to behave erratically as demonstrated.


\error{Include BA Shapes concepts and those provided by Magister et al. for full comparison.}
